...
def get_args_parser():
    ...
    parser.add_argument('--log_dir', default='./test_log',
                        help='path where to tensorboard log')
    parser.add_argument('--vis_name', default='Baseline_Espm',
                        help='name to save the vis dir')

    parser.add_argument('--device', default='cuda:2',
                        help='device to use for training / testing')
    parser.add_argument('--seed', default=0, type=int)
    parser.add_argument('--resume', default='./results/checkpoint-single-191.pth',
...
class TestData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pt_map = torch.from_numpy(gt_map).clone()

        gt_map = ndimage.gaussian_filter(gt_map, sigma=(1, 1), order=0)
        gt_map = torch.from_numpy(gt_map)
        gt_map = gt_map *60
        
        sample = {'image':image,'dots':dots, 'boxes':boxes, 'pos':rects, 'gt_map':gt_map, 'scale':scale, 'pt_map':pt_map}

        return sample['image'], sample['dots'], sample['boxes'], sample['pos'] ,sample['gt_map'],im_id,sample['scale'], sample['pt_map']
        # return sample['image'], sample['dots'], sample['boxes'], sample['pos'], sample['gt_map'], im_id, sample['scale'], sample['pt_map'], sample['gt_map']
...
def main(args):
    ...
    for data_iter_step, (samples, gt_dots, boxes, pos, gt_map, im_id, scale, pt_map) in enumerate(metric_logger.log_every(data_loader_test, print_freq, header)):
        ...
        pt_map = pt_map.to(device, non_blocking=True)
        ...
        if s_cnt >= 1:
            ...
            for r_image in r_images:
                ...
                with torch.no_grad():
                    while start + 383 < w:
                        input_x = [r_image[:,:,:,start:start+384], boxes,scale, im_id]
                        output, attnl, sim = model(input_x)
                        # output, prior, attns = model(input_x)
        ...
        print(f'{data_iter_step}/{len(data_loader_test)}: imid: {im_id}, pred_cnt: {pred_cnt},  gt_cnt: {gt_cnt},  error: {cnt_err},  AE: {cnt_err},  SE: {cnt_err ** 2} ')
    ...
    print('Low MAE: {:5.2f}, RMSE: {:5.2f} '.format( low_mae/low_count, (low_rmse/low_count)**0.5))  # low_count: 605
    print('High MAE: {:5.2f}, RMSE: {:5.2f} '.format( high_mae/high_count, (high_rmse/high_count)**0.5))  # high_count: 585
    print(low_count, high_count, len(data_loader_test))
